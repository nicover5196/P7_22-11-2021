  ...
  likePost: function(req, res) {
    ...
    asyncLib.waterfall([
      function(done) {
        ...
        .catch(function(err) {
          return res.status(500).json({ 'Erreur': 'Impossible de verifiée le message' });
        });
      },
      function(messageFound, done) {
        if(messageFound) {
          ...
          .catch(function(err) {
            return res.status(500).json({ 'Erreur': 'Impossible de vérifiée l utilisateur' });
          });
        } else {
          res.status(404).json({ 'Erreur': 'Publication déjà like' });
        }
      },
      ...
      function(messageFound, userFound, userAlreadyLikedFound, done) {
        if(!userAlreadyLikedFound) {
          ...
          if (userAlreadyLikedFound.isLike === DISLIKED) {
            ...
            res.status(409).json({ 'Erreur': 'message already liked' });
          }
        }
      },
      function(messageFound, userFound, done) {
        messageFound.update({
          ...
          res.status(500).json({ 'Erreur': 'cannot update message like counter' });
        });
      },
